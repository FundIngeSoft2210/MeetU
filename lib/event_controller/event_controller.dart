...
class EventController {
  ...
  Stream<DocumentSnapshot<Map<String, dynamic>>> currentUser(String userId) {
    return _i_dbProvider.currentUser(userId);
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getStudentGroups(
      String studentId) {
    return _i_dbProvider.getStudentGroups(studentId);
  }

  Future<DocumentSnapshot<Map<String, dynamic>>> getGroup(
      String groupId) async {
    return await _i_dbProvider.getGroup(groupId);
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getGroupChat(String groupId) {
    return _i_dbProvider.getGroupChat(groupId);
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getAllGroups() {
    return _i_dbProvider.getAllGroups();
  }

  Stream<DocumentSnapshot<Map<String, dynamic>>> getGroupStream(
      String groupId) {
    return _i_dbProvider.getGroupStream(groupId);
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getGroupPosts(String groupId) {
    return _i_dbProvider.getGroupPosts(groupId);
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getGroupEvents(String groupId) {
    return _i_dbProvider.getGroupEvents(groupId);
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getStudentEvents(
      String studentId) {
    return _i_dbProvider.getStudentEvents(studentId);
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getGroupMembers(String groupId) {
    return _i_dbProvider.getGroupMembers(groupId);
  }

  Future<bool> updateStudent(Student student) {
    return _i_dbProvider.updateStudent(student);
  }

  Future<bool> deleteUser(String userId) {
    return _i_dbProvider.deleteUser(userId);
  }
  ...
  studentSignUp(
    ...
    try {
      UserCredential credential =
          await _i_auth.createUserWithEmailAndPassword(email, password);
      Student newStudent = Student();
      _i_signUp.initMainInfo(email, name, lastName, credential.user!.uid,
          Career(semester, careerType), gender, newStudent);
      await sendVerificationEmail();
      Navigator.of(context).pushReplacement(MaterialPageRoute(
          builder: (context) => VerifyEmailScreen(student: newStudent)));
    } on FirebaseAuthException catch (e) {
  ...
  sendGroupMessage(String message, Student student, GroupChat groupChat) async {
    _i_message.sendGroupMessage(message, groupChat, student);
  }

  //POST
  add_Post(Student student, String groupid, String description, File? file,
  ...
  //EVENTS
  addEvent(String groupid, String description, String place, DateTime date,
      Student student) async {
    await _i_groupEvent.addEvent(groupid, description, place, date, student);
  }
}
